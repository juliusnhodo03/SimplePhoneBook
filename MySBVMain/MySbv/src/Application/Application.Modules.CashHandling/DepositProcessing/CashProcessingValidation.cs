...
{
    public class CashProcessingValidation : ICashDepositProcessing
    {
        #region Fields
        ...
        public CashProcessingValidation(IRepository repository, IMapper mapper, ILookup lookup,
            ISecurity security, IUserAccountValidation userAccountValidation, IMsmqConnector queueConnector)
        {
        ...
        public MethodResult<CashProcessingDto> FindBySealSerialNumber(string sealSerialNumber, string username)
        {
            ...
            if (container != null)
            {
                ...
                // reject processing.
                var productType = _repository.Query<ProductType>(a => a.LookUpKey == "MYSBV_VAULT").FirstOrDefault();
                var status = _lookup.GetStatusId("PAY_UNCONFIRMED");

                if (container.CashDeposit.ProductTypeId == productType.ProductTypeId && container.CashDeposit.StatusId != status && container.CashDeposit.CitDateTime == null)
                    return new MethodResult<CashProcessingDto>(MethodStatus.Error, null,
                        "You can only process a vault deposit after CIT is complete.");


                var drops = container.ContainerDrops.Where(a => a.IsNotDeleted).ToList();
	            ...
	            var containers = cashDepositDto.Containers.Where(e => e.IsNotDeleted);
                ...
                switch (loggedOnUser.UserType.LookUpKey)
                {
                    ...
                    default: return new MethodResult<CashProcessingDto>(MethodStatus.Error, null, "Retail users cannot process deposits.");
                }
            }
        ...
        public MethodResult<bool> Process(CashProcessingDto processingDto, string username)
        {
            ...
            {
                cashDeposit.SettlementIdentifier = ApplicationHelpers.GenerateSettlementIdentifier(DbTable.Cashdeposit, _repository);
            }
            ...
            foreach (var container in cashDeposit.Containers)
            {
                ...
                foreach (var drop in container.ContainerDrops)
                {
                    ...
                    if (cashDeposit.ProductTypeId == productTypeId &&
                        cashDeposit.DepositTypeId == _lookup.GetDepositType("MULTI_DEPOSIT").DepositTypeId)
                    {
                        drop.SettlementIdentifier = ApplicationHelpers.GenerateSettlementIdentifier(DbTable.MultiDeposit, _repository);
                    }
            ...
            var result = _repository.Update(cashDeposit) > 0;
                ...
                foreach (var item in container.ContainerDrops.SelectMany(e => e.ContainerDropItems))
                {
                    ...
                    foreach (var vaultItem in container.ContainerDrops.SelectMany(e => e.ContainerDropItems))
                    {
                        if (item.DenominationId == vaultItem.DenominationId)
                        {
                            ...
                            containerDrop.Count += vaultItem.Count;
                        }
                    }
        ...
        /// <returns></returns>
        private IEnumerable<VaultContainerDropDto> RemoveDuplicateVaultItems(IList<VaultContainerDropDto> drops)
        {
        ...
        private Collection<ContainerDropItem> OrderDescendingContainerDropItems(IEnumerable<ContainerDropItem> items)
        {
            ...
            var entire = items.OrderByDescending(e => e.ValueInCents);

            foreach (var item in entire)
            {
                collection.Add(item);
            }
            return collection;
                ...
                foreach (var containerDropItem in cashDeposit.VaultContainerDrops)
                {
                    switch (containerDropItem.ValueInCents)
                    {
                        case 1000:
                            ...
                            confirmationMessage.Counted10 = containerDropItem.ActualCount.Value;
                            break;

                        case 2000:
                            confirmationMessage.Declared20 = containerDropItem.Count;
                            confirmationMessage.Counted20 = containerDropItem.ActualCount.Value;
                            break;

                        case 5000:
                            confirmationMessage.Declared50 = containerDropItem.Count;
                            confirmationMessage.Counted50 = containerDropItem.ActualCount.Value;
                            break;

                        case 10000:
                            confirmationMessage.Declared100 = containerDropItem.Count;
                            confirmationMessage.Counted100 = containerDropItem.ActualCount.Value;
                            break;

                        case 20000:
                            confirmationMessage.Declared200 = containerDropItem.Count;
                            confirmationMessage.Counted200 = containerDropItem.ActualCount.Value;
                            break;
                    }
                }
